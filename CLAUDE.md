...
| 10 | Player depth | Real brightness (window)/volume (AudioManager) drag gestures, real audio-track picker (`audioOptions` from ExoPlayer `Tracks` + `TrackSelectionOverride`), Picture-in-picture (`enterPip`, manifest `supportsPictureInPicture`), tap-anywhere controls, double-tap edge seek, long-press 2× fast-forward (release restores), in-player playlist sheet, BackHandler closes player (progress saved → Continue). Season-variant labels ("Season 1 · Korean Language" vs "· Hindi Dubbed"; variants don't chain in autoplay). **Verified live: audio picker (real Hindi MP4A 2ch), PiP floating window, variant chips.** Background playback (MediaSessionService) deferred. | **done** (verified on device) |
